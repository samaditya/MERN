...
router.put(
    ...
    async (req, res) => {
      ...
      try {
        ...
        console.error(err.message);
        res.status(500).send('Server Error at exp put');
      }
    }
  );
...
router.delete('/experience/:exp_id' , auth , async(req , res) =>{
    try {
        ...
        console.error(err);
        return res.status(500).send("Server Error at delete experience")
...
router.put(
    ...
    check('fieldofstudy', 'field of study is reqd')
      .notEmpty(),
    async (req, res) => {
      const errors = validationResult(req);
      if (!errors.isEmpty()) {
...
router.delete('/education/:edu_id' , auth , async(req , res) =>{
    try {
        ...
        console.error(err);
        return res.status(500).send("Server Error at delete education")
...
router.get('/github/:username'  ,async(req , res) =>{
        ...
        console.error(err.message);
        return res.status(500).send('server error at github')
